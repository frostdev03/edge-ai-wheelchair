...
//       paketKirim.data_suara[i] = (int16_t)(raw_samples[i] >> 14);  //14 deflt, 11 tinggi, 16 aman
//       // paketKirim.data_suara[i] = (int16_t)(raw_samples[i] >> 11);

//     }


//     // Di dalam loop TX, setelah membaca raw_samples:
//     static int32_t dc_offset = 0;
//     for (int i = 0; i < sampleCount; i++) {
//       dc_offset = dc_offset - (dc_offset >> 8) + raw_samples[i];  // IIR filter
//       int32_t sample_clean = raw_samples[i] - (dc_offset >> 8);
//       paketKirim.data_suara[i] = (int16_t)(sample_clean >> 11);
//     }
...
void loop() {
  ...
  if (i2s_channel_read(rx_chan, raw_samples, sizeof(raw_samples), &bytesRead, portMAX_DELAY) == ESP_OK) {
    ...
    esp_err_t hasil = esp_now_send(alamatReceiver, (uint8_t *)&paketKirim, sizeof(paketKirim));
    if (hasil == ESP_OK) paketKirim.id_paket++;
  }
}